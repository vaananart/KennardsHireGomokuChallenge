...
{
	public class NorthWestSouthEastStoneAlignChecker : IRuleChecker
	{
		private readonly IDictionary<ValidatorType, IDirectionalLogicValidator> _validators;

		public NorthWestSouthEastStoneAlignChecker(IDictionary<ValidatorType, IDirectionalLogicValidator> validators)
		{
										...
										, IEnumerable<StoneModel> sameColouredStones)
		{
			if (matchingNeighbours.Any(x => x.direction == DirectionType.NorthWest)
				&& (matchingNeighbours.Any(x => x.direction == DirectionType.SouthEast)))
			{
				if (_validators[ValidatorType.NorthWestSouthEast].Validate(newStone, sameColouredStones))
				{
